...
@Transactional
@WithMockUser(username = "phase2-admin", roles = "ADMIN")
class Phase2ApiIntegrationTests {
        ...
        void cleanBusinessData() {
                // Các bảng con phải được xóa trước bảng cha.
                // Toàn bộ test có @Transactional nên dữ liệu local sẽ được rollback
                // sau test, không làm mất dữ liệu M5/GDELT thật.
        ...
        void supplierCrudValidationConflictAndNotFound() throws Exception {
                ...
                mvc.perform(get("/api/suppliers/" + id)).andExpect(status().isOk())
        ...
        void supplyChainRiskFlowWorksEndToEnd() throws Exception {
                ...
                mvc.perform(get("/api/purchase-orders/" + orderId)).andExpect(status().isOk())
                ...
                mvc.perform(patch("/api/purchase-orders/" + orderId + "/status").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"status\":\"ORDERED\"}")).andExpect(status().isOk())
                ...
                mvc.perform(patch("/api/shipments/" + shipmentId + "/status").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"status\":\"SHIPPING\"}")).andExpect(status().isOk())
                ...
                mvc.perform(patch("/api/risk-events/" + riskId + "/status").contentType(MediaType.APPLICATION_JSON)
                                .content("{\"status\":\"INVESTIGATING\"}")).andExpect(status().isOk())
