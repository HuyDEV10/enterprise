    ...
    @Bean SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable()).cors(cors -> cors.configurationSource(corsConfigurationSource()))
            ...
            .authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers(HttpMethod.GET, "/api/alerts/**").hasAnyRole("ADMIN", "MANAGER", "ANALYST")
                .requestMatchers(HttpMethod.POST, "/api/alerts/**").hasAnyRole("ADMIN", "MANAGER")
                .requestMatchers(HttpMethod.DELETE, "/api/alerts/**").hasAnyRole("ADMIN", "MANAGER")
                .requestMatchers("/api/alerts/**").hasAnyRole("ADMIN", "MANAGER", "ANALYST")
                .requestMatchers(HttpMethod.GET, "/api/companies/**", "/api/departments/**", "/api/employees/**").hasAnyRole("ADMIN", "MANAGER", "ANALYST")
                .requestMatchers("/api/companies/**", "/api/departments/**", "/api/employees/**").hasRole("ADMIN")
                .requestMatchers(HttpMethod.GET, "/api/**").hasAnyRole("ADMIN", "MANAGER", "STAFF", "ANALYST")
